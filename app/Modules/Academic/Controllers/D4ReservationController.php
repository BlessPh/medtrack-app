...
class D4ReservationController
{
    ...
    public function requests(Request $request): JsonResponse
    {
        ...
        return response()->json(['data' => CampaignHospital::query()->where('hospital_id', $hospital->id)->whereHas('campaign', fn ($query) => $query->where('strategy', 'D4_RESERVATION'))->with(['campaign.academicYear', 'campaign.promotions.level', 'campaign.media', 'media'])->latest('requested_at')->get()]);
    }
    ...
    public function respond(Request $request, CampaignHospital $campaignHospital): JsonResponse
    {
        ...
        abort_unless($campaignHospital->campaign->strategy === 'D4_RESERVATION' && $campaignHospital->request_status === 'PENDING', 409, 'Cette demande a déjà reçu une réponse.');
    ...
    public function studentView(Request $request, Campaign $campaign, EligibilityService $eligibility): JsonResponse
    {
        ...
        $campaign->load(['academicYear', 'promotions.level', 'media', 'hospitals' => fn ($query) => $query->where('request_status', 'ACCEPTED')->with(['hospital', 'capacityPools'])]);
        ...
        $hospitals = $campaign->hospitals->map(function ($item): array {
            ...
            return ['id' => $item->id, 'hospital' => ['id' => $item->hospital->public_id, 'name' => $item->hospital->name], 'capacity' => $pool?->total_places ?? 0, 'reserved' => $pool?->reserved_places ?? 0, 'remaining' => max(0, ($pool?->total_places ?? 0) - ($pool?->reserved_places ?? 0))];
        });
    ...
    public function reserve(Request $request, Campaign $campaign, EligibilityService $eligibility): JsonResponse
    {
        $data = $request->validate(['hospital_id' => ['required', 'uuid']]);
        $student = Student::where('user_id', $request->user()->id)->firstOrFail();
        abort_unless($campaign->strategy === 'D4_RESERVATION' && $eligibility->isEligible($student, $campaign), 422, 'Cette campagne n’est pas disponible pour cet étudiant.');
        ...
        $application = DB::transaction(function () use ($campaign, $student, $hospitalId): Application {
            ...
            if ($application && ! in_array($application->status, ['WITHDRAWN', 'CANCELLED'], true)) throw ValidationException::withMessages(['reservation' => 'Une réservation active existe déjà pour cette campagne.']);
            ...
            if ($pool->reserved_places >= $pool->total_places) throw ValidationException::withMessages(['hospital_id' => 'Cet hôpital ne dispose plus de place.']);
            $values = ['campaign_id' => $campaign->id, 'student_id' => $student->id, 'preferred_hospital_id' => $hospitalId, 'status' => 'RESERVED', 'submitted_at' => now(), 'reviewed_at' => null, 'reviewed_by' => null, 'review_note' => null];
            $application ? $application->update($values) : $application = Application::create($values);
            $pool->increment('reserved_places');
            CapacityReservation::updateOrCreate(['application_id' => $application->id], ['capacity_pool_id' => $pool->id, 'status' => 'HELD', 'expires_at' => null]);
            return $application;
        }, 3);
